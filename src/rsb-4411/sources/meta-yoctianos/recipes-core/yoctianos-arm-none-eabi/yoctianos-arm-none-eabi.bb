...
do_install() {
    ...
    # Extract each .deb manually using ar + tar
    for pkg in ${WORKDIR}/*.deb; do
        [ -e "$pkg" ] || { echo "ERROR: No .deb files found in WORKDIR"; exit 1; }
        ...
        for archive in data.tar.*; do
            tar -xf "$archive" -C ${D}/opt/yoctianos-arm-none-eabi/
        done

        # Cleanup extracted metadata
